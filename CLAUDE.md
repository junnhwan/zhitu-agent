...
- **MCP Server 对外暴露**：由 `mcp.server.enabled` 开关（默认关）。启用后必须配 `mcp.server.auth_token`（推荐走 `ZHU_MCP_SERVER_AUTH_TOKEN` 环境变量），否则启动 fail-fast。Streamable HTTP 单端点挂到现有 Gin 同端口（默认 `/mcp`），`middleware.BearerAuth` 用 constant-time 比较。默认只暴露 `getCurrentTime` / `addKnowledgeToRag` / `retrieveKnowledge`（新增，包 `rag.RAG.Retriever`）。**sendEmail 不暴露**（副作用 + 无远端审计）。Hooks 注入 metrics（`mcp_server_tools_total` / `mcp_server_calls_total` / `mcp_server_call_duration_seconds` / `mcp_server_unauth_total`）。`main.go` 关停时调 `mcpSrv.Shutdown(ctx)`。
